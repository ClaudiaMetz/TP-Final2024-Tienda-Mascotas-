...
async function loadData(){
  try{
    const res = await fetch('adoptables.json', {cache: "no-store"});
    adoptables = await res.json();
    render(adoptables);
  }catch(e){
    console.error("No pude cargar adoptables.json", e);
    listaContainer.innerHTML = "<p class='text-danger'>Error cargando adoptables.</p>";
  }
}
    ...
    col.innerHTML = `
      <div class="card shadow-sm h-100">
        ...
        <div class="card-body d-flex flex-column">
          <h5 class="card-title fw-bold">${a.nombre}</h5>
          <p class="mb-1 small">Tipo: ${a.tipo} • ${a.tam} • ${a.edad}</p>
          <p class="mb-3">${a.desc || ''}</p>
          <div class="mt-auto">
            <button class="btn btn-outline-warning w-100 ver-btn" data-id="${a.id}">Ver más</button>
          </div>
        </div>
      </div>`;
...
filtrarBtn.addEventListener('click', ()=>{
  ...
  const filtrado = adoptables.filter(a=>
    (t === '' || a.tipo === t) &&
    (s === '' || a.tam === s) &&
    (e === '' || a.edad === e)
  );
  ...
  if(filtrado.length === 0){
    alert("No hay adoptables que coincidan con esos filtros. Pawly recomienda ampliar un poco la búsqueda.");
    render(adoptables);
    return;
  }
  render(filtrado);
